...
const MOCK_AIRDROPS: TokenAirdrop[] = [
  {
    ...
    description: 'Airdrop for TON holders! Participate and get your Toncoin.',
    requirements: [
      {
        type: 'telegram',
        url: 'https://t.me/toncoin_official',
        description: 'Subscribe to the official TON Telegram',
      },
      {
        type: 'website',
        url: 'https://ton.org/',
  ...
  },
  {
    ...
    description: 'NOTCOIN giveaway for all Notcoin Telegram subscribers!',
    requirements: [
      {
        type: 'telegram',
        url: 'https://t.me/notcoin',
        description: 'Subscribe to Notcoin Telegram channel',
      },
      {
        type: 'twitter',
        url: 'https://twitter.com/notcoin_crypto',
        description: 'Follow Notcoin on Twitter',
      },
    ],
  ...
  },
  {
    ...
    description: 'Jetton for active users! Don’t forget to complete all tasks.',
    requirements: [
      {
        type: 'website',
        url: 'https://jetton.games/',
        description: 'Register on Jetton website',
      },
    ],
...
const Airdrop: React.FC<{ creatorMode: boolean }> = ({ creatorMode }) => {
  ...
  const handleClaimAirdrop = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        alert('Failed to claim tokens. Please check that you have met all the requirements.');
      }
    } catch (error) {
  ...
  const handleEndAirdrop = async () => {
    if (!walletAddress || !selectedAirdrop) return;
    if (!window.confirm('Are you sure you want to end the airdrop? This action cannot be undone.')) return;
    setEndLoading(true);
    try {
      const success = await mockEndAirdrop();
      if (success) {
        alert('Airdrop successfully ended!');
        closeDetailsModal();
      } else {
        alert('Failed to end airdrop.');
      }
    } catch (error) {
      alert('An error occurred while ending the airdrop');
    } finally {
      setEndLoading(false);
    }
  };
        ...
        <div className="airdrop-wrapper">
          <div className="airdrop-header">
            <h1 className="airdrop-title">
              Airdrop {creatorMode ? 'for Token Creators' : 'for Traders'}
            </h1>
            {creatorMode && (
              <button onClick={navigateToCreateAirdrop} className="create-button">Create Airdrop</button>
            )}
          </div>
          ...
          {!walletAddress ? (
            <div className="card">
              ...
              <p className="card-text">You need to connect your wallet to view available airdrops.</p>
            </div>
          ) : loading ? (
            <div className="loading-spinner-container"><div className="loading-spinner"></div></div>
          ) : airdrops.length === 0 ? (
            <div className="card">
              <h2 className="card-title">No Airdrops Available</h2>
              <p className="card-text">
                {creatorMode
                  ? 'You have not created any airdrops yet. Create a new airdrop to start distributing tokens.'
                  : 'There are currently no available airdrops. Please check back later.'}
              </p>
            </div>
          ) : (
          ...
          <div className="modal-content">
            <div className="modal-inner">
              ...
              <div className="modal-body">
                ...
                <div>
                  <h3 className="requirements-title">Requirements to receive tokens</h3>
                    ...
                    {selectedAirdrop.requirements.map((req, index) => (
                      <div key={index} className="requirement-item">
                        ...
                        <a href={req.url} target="_blank" rel="noopener noreferrer" className="requirement-link">
                          {req.type === 'twitter' && 'Go to Twitter'}
                          {req.type === 'telegram' && 'Go to Telegram'}
                          {req.type === 'discord' && 'Go to Discord'}
                          {req.type === 'website' && 'Go to website'}
                          {!['twitter', 'telegram', 'discord', 'website'].includes(req.type) && 'Go to link'}
                        </a>
                      </div>
                    ))}
                  </div>
                ...
                {!creatorMode && (
                  <button className={`claim-button ${claimLoading ? 'loading' : ''} ${requirementsStatus && !requirementsStatus.completed ? 'disabled' : ''}`}
                    ...
                    {claimLoading ? 'Claiming...' : requirementsStatus && !requirementsStatus.completed ? 'Complete all requirements' : 'Claim tokens'}
                  </button>
                )}
                {creatorMode && (
                  <div className="button-group">
                    <button className="edit-button" onClick={() => navigateToEditAirdrop(selectedAirdrop.id)}>Edit airdrop</button>
                    <button className={`end-button ${endLoading ? 'loading' : ''}`} onClick={handleEndAirdrop} disabled={endLoading}>
                      {endLoading ? 'Ending...' : 'End airdrop'}
                    </button>
                  </div>
                )}
              </div>
